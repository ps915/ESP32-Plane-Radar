 ...
 * far target never steals a slot from a nearby one. `types`/`altitude_in_meters`
...
bool fetchFromSource(WiFiClient& client, const char* url, bool server_filtered,
                     ...
                     const char* label, const TypeFilter& types,
...
bool fetchUpdate(double center_lat, double center_lon, float fetch_radius_km,
  ...
  if (s_local_url[0] != '\0') {
    ...
    if (fetchFromSource(client, s_local_url, /*server_filtered=*/false,
                        ...
                        fetch_radius_km, "local", types, altitude_in_meters)) {
      s_using_local = true;
      return true;
    }
  }
  ...
  if (fetchFromSource(client, url.c_str(), /*server_filtered=*/true,
                      ...
                      "adsb.fi", types, altitude_in_meters)) {
    s_using_local = false;
    return true;
  }
